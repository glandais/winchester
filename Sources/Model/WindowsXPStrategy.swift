...
struct WindowsXPStrategy: DefragStrategy {
    ...
    func plan(volume input: DefragVolume) -> DefragPlan {
        ...
        for position in candidates {
            ...
            guard let target = firstGap(in: volume, need: file.clusterCount, total: total) else {
    ...
    private func firstGap(in volume: DefragVolume, need: UInt32, total: UInt32) -> Extent? {
        var cursor: UInt32 = 0
        while cursor < total {
