...
#
# `full` fait les vingt démarrages, les vingt installations, les quatre journées
# du README, les vingt profils croisés avec les treize outils, et XP et
# UltraDefrag en blocs pleins sur les huit NTFS : 320 bilans. Chaque bilan est
# un fichier texte dans $MEASURE_DIR/out-<étape>/.
set -e
cd "$(dirname "$0")/../.."
STEP="${1:?usage : run.sh <étape> boots|full}"
WHAT="${2:-boots}"
